...
class ExtractProperty(BaseModel):
    ...
    items: Optional[Union[List, Dict[str, Any]]] = None  # 添加默认值
    enum: Optional[List[str]] = None  # 添加默认值
    required: bool = True
...
class DoctranConfig(BaseModel):
    openai_deployment_id: Optional[str] = None  # 设置为可选，默认为 None
    openai_model: str
    openai: Any
...
class Doctran:
    def __init__(self, 
        ...
        self.config = DoctranConfig(
            ...
            openai_deployment_id=openai_deployment_id  # 添加这个参数
        )
        ...
        base_url = openai_base_url or os.environ.get('OPENAI_API_BASE')
        
        # 使用新版 OpenAI SDK 初始化方式
        client_args = {"api_key": api_key}
        if base_url:
            client_args["base_url"] = base_url
            
        self.config.openai = OpenAI(**client_args)
